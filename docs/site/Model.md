...
{% include note.html content="If you used LB3 before, the model decorator in LoopBack 4 is not exactly the same as what it is
in LoopBack 3. For example, properties and relation cannot be defined through the model decorator. Please check the following section for available entires." %}
...
-->
  ...
  <tr>
    <td><code>setting.scope</code></td>
    <td>Object</td>
    <td>N/A</td>
    <td>Scope enable you to specify commonly-used queries that you can reference as method calls on a model. See <a href="#scope">Scope</a> below for more details and examples.</td>
  </tr>
...
_Scope_ enable you to specify commonly-used queries that you can reference as method calls on a model.

If you wish for a scope to be applied across all queries to the model, you can set, for example:
...
```ts
await Product.find({offset: 0});
```

Becomes the equivalent of this:

```ts
await Product.find({order: "name", offset: 0, limit: 100, where: {deleted: false}})
```
...
LoopBack 4 expects a model to have one _ID properties_ that uniquely identify the model instance.
...
In LoopBack, [auto-migration](Database-migrations.md) helps the user create relational database schemas based on definitions of their models. Here are some id property settings that can be used for auto-migration / auto-update:
...
<table>
  ...
  <tbody>   
    <tr>
      ...
      <td>No</td>
      <td>Boolean</td>
      <td><strong>For auto-migrate usage</strong>. The <code>generated</code> property indicates the ID will be automatically generated by the database. When it sets to true, the value of the id property will be generated by the database automatically with its default type (e.g integer for MySQL and string for MongoDB).</td>
    </tr>
    <tr>
      <td>useDefaultIdType</td>
      <td>No</td>
      <td>Boolean</td>
      <td><strong>For auto-migrate usage</strong>. Set it to false when it's needed to auto-generate non-default type property values. For example, for PostgreSQL, to use uuid as the id property, the id type should set to string, <code>generated</code> should set to true, and this field should be set to false. Please check each connector's README file for more information about auto-migration/auto-update.</td>
    </tr>
  </tbody>
</table>
...
4.  Checkout [Database Migration](Database-migrations.md) if you'd like to have LoopBack 4 to create relational database schemas for you based on model definitions. Always check [Database Connectors](Database-connectors.md) for details and examples for database migration / discover.
